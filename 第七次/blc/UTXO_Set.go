	...
	for _,tx := range block.YX_Txs  {
		...
		for index,out := range tx.Vouts  {
			...
			for _,in := range ins  {
				...
				if in.Vout == index && bytes.Compare(tx.TxHash ,in.Txhash) == 0 && bytes.Compare(out.PubKeyHash,Ripemd160Hash(in.PublicKey)) == 0 {
		...
		if b != nil {
			...
			for _,in := range ins {
				...
				for _,utxo := range txOutputs.UTXOS  {
					...
					if in.Vout == utxo.Index && bytes.Compare(utxo.Output.PubKeyHash,Ripemd160Hash(in.PublicKey)) == 0 {
